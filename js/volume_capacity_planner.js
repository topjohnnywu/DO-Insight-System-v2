        ...
        if (strictLooseItems.length > 0) {
            ...
            for (let item of tvLooseItems) {
                ...
                while (remainingQty > 0) {
                    ...
                    let exceedWidth = currentTvWidthAccum + itemW > 3.65; // Max pallet width is ~3.6ft
                    ...
                    if (currentTvPallet.length > 0 && percentPerItem < 0.15) {
                        ...
                        exceedWidth = currentTvWidthAccum + itemW > 4.5; // Allow extra width (squeezing or turning sideways)
                    }
                    
                    if ((exceedFraction || exceedWidth) && currentTvPallet.length > 0) {
            ...
            for (let item of otherLooseItems) {
                ...
                while (remainingQty > 0) {
                    ...
                    for (let mp of mixedPallets) {
                        ...
                        if (mp.isTvMixed && (mp.fillPercent + percentPerItem <= maxFill)) {
        ...
        this.renderPalletsLayout = () => {
            ...
            this.palletsToPlace.forEach((p, index) => {
                ...
                if (!p.isMixed && p.tie && p.high) {
                    ...
                    const ties = String(p.tie).split(',').map(s => parseInt(s.trim()));
                    const highs = String(p.high).split(',').map(s => parseInt(s.trim()));
                ...
                if (p.isTvMixed && p.tvItems && p.tvItems.length > 0) {
                    ...
                    let maxTvHeight = 0; // Track the tallest TV on this pallet
                    ...
                    // Flatten all individual TV items into a sequential list
                    let allTvUnits = [];
                    for (let tvItem of sortedTvItems) {
                        let tvQty = tvItem.qty;
                        let cW_cm = tvItem.cartonW;
                        let cL_cm = tvItem.cartonL;
                        let cH_cm = tvItem.cartonH;
                        
                        let tie = tvItem.tie;
                        let high = tvItem.high || 1;
                        ...
                        if (!cH_cm) {
                            ...
                            if (cH_cm > (palletH - baseHeight) * 30.48) {
                                cH_cm = (palletH - baseHeight) * 30.48 / high;
                            }
                        }
                        
                        if (cW_cm > cL_cm) {
                            const temp = cL_cm; cL_cm = cW_cm; cW_cm = temp;
                        }
                        ...
                        if (isNaN(cH) || cH <= 0) cH = (palletH - baseHeight);
                        
                        if (cH > maxTvHeight) maxTvHeight = cH;
                        
                        for (let q = 0; q < tvQty; q++) {
                            allTvUnits.push({
                                item: tvItem,
                                cW: cW,
                                cL: cL,
                    ...
                    // Ghost box for HIFI (other items) top-loaded on TV pallet
                    if (p.otherItems && p.otherItems.length > 0) {
                        let ghostD = buildL * 0.8; // 80% of pallet depth
                        let ghostW = buildW * 0.8; // 80% of pallet width
                        let ghostH = 45 / 30.48; // Fixed 45cm height for accessories

                        if (maxTvHeight <= 0) maxTvHeight = (palletH - baseHeight); 
                        
                        const ghostGeo = new THREE.BoxGeometry(ghostW, ghostH, ghostD);
                        const ghostMat = createCartonMaterial('HIFI', '', hifiColor);
                        ...
                        // Sit directly on top of the TVs, centered
                        ghostMesh.position.set(0, baseHeight + maxTvHeight + (ghostH / 2), 0);
